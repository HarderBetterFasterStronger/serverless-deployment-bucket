...
class DeploymentBucketPlugin {
  constructor(serverless, options) {
    ...
    if (this.deploymentBucket.name) {
      ...
      this.config.blockPublicAccess = get(this.config, "blockPublicAccess", false)
  ...
  async hasPublicAccessBlock(name) {
    const params = {
      Bucket: name,
    }

    try {
      await this.provider.request("S3", "getPublicAccessBlock", params)
      return true
    } catch (e) {
      return false
    }
  ...
  async putPublicAccessBlock(name, publicAccessBlockConfiguration) {
    const params = {
      Bucket: name,
  ...
  async applyDeploymentBucket() {
    try {
      ...
      if (this.config.blockPublicAccess) {
        ...
        if (!(await this.hasPublicAccessBlock(this.deploymentBucket.name))) {
          await this.putPublicAccessBlock(this.deploymentBucket.name, publicAccessBlockConfiguration)
          this.serverless.cli.log(`Applied PublicAccessBlockConfiguration to deployment bucket`)
        }
      }
    } catch (e) {
